...
class MyDataset2(Dataset):
    def __init__(self, rootpath, list_file, transform=None, target_transform=None, loader=default_loader):
        ...
        for tmp_line in lines:
            ...
            imgs.append((rootpath + pair[0], [int(float(pair[1])), int(0.5+20*(0.55-abs(float(pair[2])-0.5)))] ))
...
if is_debug:
    train_data = MyDataset2(rootpath='', list_file='dataset3/debug_lists_weights.txt', transform=data_tf)
    train_loader = DataLoader(dataset=train_data, batch_size=batch_sz[0], shuffle=True, num_workers=4)

    val_data = MyDataset2(rootpath='', list_file='dataset3/debug_lists_weights.txt', transform=data_tf)
    val_loader = DataLoader(dataset=val_data, batch_size=batch_sz[1], shuffle=True, num_workers=4)
else:
    train_data = MyDataset2(rootpath='', list_file='dataset3/train_lists_weights.txt', transform=data_tf)
    train_loader = DataLoader(dataset=train_data, batch_size=batch_sz[0], shuffle=True, num_workers=4)

    val_data = MyDataset2(rootpath='', list_file='dataset3/val_lists_weights.txt', transform=data_tf)
    val_loader = DataLoader(dataset=val_data, batch_size=batch_sz[1], shuffle=True, num_workers=4)
...
for epoch in range(nums_epoch):
    ...
    for img , label in train_loader:
        ...
        gt_label = Variable(label[0]).cuda()
        weight = label[1].cuda()
        out = net(img)
        loss = torch.sum(criterion(out, gt_label)*weight.float()) / torch.sum(weight)
        ...
        _,pred = out.max(1)
        num_correct = (pred == gt_label).sum().item()
